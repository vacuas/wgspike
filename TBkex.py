...
def set_psk(ephem_secret, xephem_secret):
    # Compose and set WG pre-shared key
    try:
...
class PqKexServer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # Create Ephem secrets
            rxephem_key = x448.X448PrivateKey.from_private_bytes(
                os.urandom(56))
            ...
            # Compose and set WG pre-shared key
            set_psk(ephem_secret, xephem_secret)
...
def kex_request():
    ...
    logger.debug('Crypto {:.3f} ms'.format((time.time() - start_time)
                                           * 1000.0))
    start_time = time.time()
